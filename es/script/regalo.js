...
function handleGiftTap(event) {
    ...
    if (remaining === 0) {
        document.getElementById('giftText').innerHTML = '🎉 ¡El regalo se está abriendo! 🎉';
        setTimeout(() => {
        ...
        document.getElementById('giftText').innerHTML = `¡Ya casi! Solo ${remaining} clics/tocs más 💥`;
    } else if (remaining <= 7) {
        document.getElementById('giftText').innerHTML = `¡La caja se está rompiendo! ${remaining} clics/tocs más 💫`;
    } else {
        document.getElementById('giftText').innerHTML = `Toca o haz clic <span id="remainingTaps">${remaining}</span> veces para abrir el regalo 🎁`;
    }
...
addTouchSupport(closeBtn, () => {
    ...
    setTimeout(() => {
        ...
        document.getElementById('giftText').innerHTML = `Toca o haz clic <span id="remainingTaps">${maxTaps}</span> veces para abrir el regalo 🎁`;
        currentSlide = 0;
        updateCarousel();
        menuOptions.classList.remove('show');
    }, 500);
